...
# [ ] LoadJsonVisualization( "filename.json", "tool" )
# CreateVisualization( rec(
#     tool := "...",
#     data := JsonStringToGap(
#         ReadAll( InputTextFile( "anychart-sample.json" ) )
# ) );
# But how to add options?
# Re-use some code from ConvertDataSeriesForTool?
#
# [ ] PlotGraph( adjacencyMatrix, options )
# [ ] PlotGraph( vertices, edges, options )
# [ ] PlotGraph( edges, options )
# [ ] PlotGraph( vertices, edgeRelation, options )
#     layout := "name" # moved into layout.name; preset/cose are defaults
#     style := rec( ... ) # see cytoscape docs
#     group := fn on nodes
#     position := fn on nodes

LoadPackage( "JupyterViz" );

MakePlotDataSeries := function ( args... )
    ...
    if not IsRecord( args[Length( args )] ) then
        Add( args, rec() );
        return CallFuncList( MakePlotDataSeries, args );
    fi;
    ...
    if Length( args ) = 2 and IsFunction( args[1] ) then
        return MakePlotDataSeries( [1,2,3,4,5], args[1], args[2] );
    fi;
    #
...
JUPVIZ_RecordKeychainLookup := function ( record, keychain, default )
    ...
    return JUPVIZ_RecordKeychainLookup( record.( keychain[1] ),
        keychain{[2..Length(keychain)]}, default );
end;

JUPVIZ_RecordsKeychainLookup := function ( records, keychain, default )
    local record, attempt;
    for record in records do
        attempt := JUPVIZ_RecordKeychainLookup( record, keychain, default );
        if not ( attempt = default ) then return attempt; fi;
    od;
    return default;
end;

JUPVIZ_FetchWithDefault :=
function ( object, others, chain, default, action )
    if IsString( chain ) then chain := [ chain ]; fi;
    action( JUPVIZ_RecordKeychainLookup( object, chain,
        JUPVIZ_RecordsKeychainLookup( others, chain, default ) ) );
end;

JUPVIZ_FetchIfPresent := function ( object, others, chain, action )
    local result;
    if IsString( chain ) then chain := [ chain ]; fi;
    result := JUPVIZ_RecordKeychainLookup( object, chain,
        JUPVIZ_RecordsKeychainLookup( others, chain, fail ) );
    if not ( result = fail ) then action( result ); fi;
end;

ConvertDataSeriesForTool := rec();

ConvertDataSeriesForTool.plotly := function ( series )
    local result;
    result := rec( data := StructuralCopy( series ) );
    Perform( result.data, function ( s )
        JUPVIZ_FetchWithDefault( s, series, [ "options", "type" ], "line",
            function ( t ) s.type := t; end );
        JUPVIZ_FetchIfPresent( s.options, [], "name",
            function ( n ) s.name := n; end );
    end );
    JUPVIZ_FetchWithDefault( 0, series, [ "options", "height" ], 400,
        function ( h ) result.layout := rec( height := h ); end );
    JUPVIZ_FetchIfPresent( 0, series, [ "options", "width" ],
        function ( w ) result.layout.width := w; end );
    JUPVIZ_FetchIfPresent( 0, series, [ "options", "title" ],
        function ( t ) result.layout.title := t; end );
    JUPVIZ_FetchIfPresent( 0, series, [ "options", "xaxis" ],
        function ( x ) result.layout.xaxis := rec( title := x ); end );
    JUPVIZ_FetchIfPresent( 0, series, [ "options", "yaxis" ],
...
ConvertDataSeriesForTool.canvasjs := function ( series )
    ...
    JUPVIZ_FetchIfPresent( 0, series, [ "options", "width" ],
        function ( w ) result.width := w; end );
    JUPVIZ_FetchWithDefault( 0, series, [ "options", "animationEnabled" ],
        true, function ( e ) result.animationEnabled := e; end );
    JUPVIZ_FetchIfPresent( 0, series, [ "options", "theme" ],
...
JUPVIZ_PlotDataSeriesList := function ( series... )
    local result;
    JUPVIZ_FetchWithDefault( 0, series, [ "options", "tool" ], "plotly",
        function ( tool )
...
Plot := function ( args... )
    ...
    # MakePlotDataSeries.  If the last argument is a function, we assume
    # it's to be applied to the first argument, and thus again, we pass the
    # arguments directly on to MakePlotDataSeries.
    if IsRecord( args[Length( args )] ) or
       IsFunction( args[Length( args )] ) then
        return JUPVIZ_PlotDataSeriesList(
            CallFuncList( MakePlotDataSeries, args ) );
    fi;
    ...
    # directly on to MakePlotDataSeries.
    if Length( args ) = 2 and not IsList( args[1][1] ) and
       not IsFunction( args[1][1] ) then
        return JUPVIZ_PlotDataSeriesList(
            CallFuncList( MakePlotDataSeries, args ) );
    fi;
    # The only case remaining is the Plot(argset1,argset2,...) case, which
    # means we must iterate over all the arguments, call MakePlotDataSeries
    # on each, and pile them all into an array.
    return CallFuncList( JUPVIZ_PlotDataSeriesList,
        List( args, x -> CallFuncList( MakePlotDataSeries, x ) ) );
end;

ConvertGraphForTool := rec();

ConvertGraphForTool.cytoscape := function ( graph )
    return graph;
end;

PlotGraph := function ( args... )
    ...
    # For now, just throw the vertices and edges into a data structure and
    # return it.  We will make this better later.
    result := rec(
        elements := [ ],
    ...
    Perform( args[1], function ( v )
        Add( result.elements,
            rec( data := rec( id := PrintString( v ) ) ) );
    end );
    Perform( args[2], function ( e )
        Add( result.elements,
            rec( data := rec(
                source := PrintString( e[1] ), target := PrintString( e[2] )
            ) ) );
    end );
    JUPVIZ_FetchWithDefault( args[3], [], [ "tool" ], "cytoscape",
        function ( tool )
